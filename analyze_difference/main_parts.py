...
import re
from .util import get_illegal_opcode, get_tc_paths, get_illegal_tyoe
...
def rule1():
    ...
    tc_paths = get_tc_paths(path, key, tcs_base_dir)
    logs = get_logs(tc_paths)
    print(logs)
...
def rule2():
    ...
    path = '/media/ssd_wd1/cp910_data/main_testing_v125_250_9811/only_execution_log_category/log_category_only_highlight/1.json'
    tcs_base_dir = '/media/ssd_wd1/cp910_data/main_testing_v125_250_9811/diff_tcs'
    tc_paths = get_tc_paths(path, key, tcs_base_dir)[:1]
    logs = get_logs(tc_paths)
    print(logs)

def rule3():
    # ??? TODO 要查一下table.fill里为什么回报这个错/最新版本会不会报这个错/有没有可执行的table.fill
...
# TODO WASM module load failed: unsupported opcode fd加进keyword
def rule5():
    ...
    tcs_base_dir = '/media/ssd_wd1/cp910_data/main_testing_v125_250_9811/diff_tcs'
    tc_paths = get_tc_paths(path, key, tcs_base_dir)
    assert 0, print(get_logs(tc_paths[:2]))
    logs = get_logs(tc_paths, ['iwasm_fast_interp_dump'])
    iwasm_fast_logs = logs['iwasm_fast_interp_dump']
    opcodes = set()
    for log in tqdm(iwasm_fast_logs):
        opcodes.add(get_illegal_opcode(log))
    # print(opcodes)
    # {'fd', 'fc 0b'}
...
def rule4():
    # no bug
    key = '((wasm3_dump, illegal/unknown type),)'
    path = '/media/ssd_wd1/cp910_data/main_testing_v125_250_9811/only_execution_log_category/log_category_only_highlight/2.json'
    tcs_base_dir = '/media/ssd_wd1/cp910_data/main_testing_v125_250_9811/diff_tcs'
    tc_paths = get_tc_paths(path, key, tcs_base_dir)[:1]
    logs = get_logs(tc_paths)
    print(logs)
...
def rule7():
    # bug / wasm3
    # TODO check 下 {'fc 0a', 'fc 0b'}这两个指令对应什么，具体是怎么错的
    ...
    tc_paths = get_tc_paths(path, key, tcs_base_dir)
    logs = get_logs(tc_paths[:1], process=False)
    print('one example:')
    print(logs)
    logs = get_logs(tc_paths, ['iwasm_fast_interp_dump'])
    iwasm_fast_logs = logs['iwasm_fast_interp_dump']
...
def rule9():
    ...
    illegal_types = set()
    for log in tqdm(iwasm_fast_logs):
        illegal_types.add(get_illegal_tyoe(log))
...
def rule11():
    # bug / wasm3
    # TODO check 下 {'c7', 'f0', 'e1', 'ef', 'dd', 'ec'}这两个指令对应什么，具体是怎么错的
...
# ((WAVM_default, <function or section size mismatch>), (WasmEdge_disableAOT_newer, <function or section size mismatch>), (iwasm_classic_interp_dump, <function or section size mismatch>), (iwasm_fast_interp_dump, <function or section size mismatch>), (wasmer_default_dump, <function or section size mismatch>), (wasmi_interp, <function or section size mismatch>))
...
def rule15():
    ...
    tc_paths = []
    for key in keys:
        tc_paths.extend(get_tc_paths(path, key, tcs_base_dir))
    dir_ = check_dir('tt/wats')
    for p in tc_paths:
        wasm2wat(p, 'tt/tt.wat')
        assert path_read('tt.wat').find('call 0')
...
def rule16():
    ...
    print('one example:')
    print(logs)
    logs = get_logs(tc_paths, ['wasmer_default_dump'])
    iwasm_fast_logs = logs['wasmer_default_dump']
    opcodes = set()
    for log in tqdm(iwasm_fast_logs):
        opcodes.add(get_wasmer_illegal_opcode(log))
    print(opcodes)
...
def rule20():
    ...
    # key = '((wasm3_dump, illegal/unknown opcode),)'
    path = 'some_logs/only_highlight_log_category/8.json'
    tcs_base_dir = '/media/ssd_wd1/cp910_data/main_testing_v125_350_9811/diff_tcs'
    tc_paths = get_tc_paths(path, key, tcs_base_dir)
    print(len(tc_paths))
    logs = get_logs(tc_paths[:], process=False)
    print('one example:')
    print(logs)
    # logs = get_logs(tc_paths, ['wasmer_default_dump'])
    # iwasm_fast_logs = logs['wasmer_default_dump']
...
# =========================================================
# * 下面这个典型
# ((iwasm_classic_interp_dump, (CannotExecute,)), (iwasm_fast_interp_dump, (CannotExecute,)))
# "((iwasm_classic_interp_dump, invalid local count), (iwasm_fast_interp_dump, invalid local count))"
