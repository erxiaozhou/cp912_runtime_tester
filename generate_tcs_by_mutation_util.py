...
from generate_wasm_tc_util import get_wasm_bytes_from_dict, prepare_template, read_next_leb_num, write_wasm_from_dict
...
def generate_tcs(ori_tc_path, mutate_num, new_tc_dir):
    ...
    for new_path in paths:
        # print(new_path, len(paths), '==============================')

        tc_bytes = _generate_one_tc_bytes(sec_template, code_sec_bytes, masks)
        # assert 0
        write_bytes(new_path, tc_bytes)
    return paths


def _generate_one_tc_bytes(sec_template, code_sec_bytes, masks):
    sec_template['code'] = _generate_new_code_sec(code_sec_bytes, masks)
    return get_wasm_bytes_from_dict(sec_template)
